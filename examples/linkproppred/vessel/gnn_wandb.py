...
def main():
    ...
    wandb.init(config=args, project="ogbl-vesselpatch-gnn-baseline")

    device = f'cuda:{args.device}' if torch.cuda.is_available() else 'cpu'
    device = torch.device(device)

    dataset = PygLinkPropPredDataset('ogbl-vessel_patch', 
    ...
    if args.use_node_embedding:
        data.x = torch.cat([data.x, torch.load('embedding_patch.pt')], dim=-1)
    data = data.to(device)
    ...
    evaluator = Evaluator(name='ogbl-vessel_patch')
